...
import org.lotka.xenonx.data.model.remote.user.UserItemRemote
import org.lotka.xenonx.data.model.remote.user.UserListItemRemote
...
//    }
    ...
    override fun observeUserChatList(page: Int): Flow<ResultState<ChatListResponseModel>> =
        callbackFlow {
            ...
            val eventListener = object : ChildEventListener {
                override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                    try {
                        ...
                        Log.d(
                            "pshiiisagi",
                            "onChildAdded excepion:  ${e.message} and ${e.localizedMessage} and ${e.stackTrace} and ${e.printStackTrace()}"
                        )
                    }
                }
                ...
                override fun onChildChanged(snapshot: DataSnapshot, previousChildName: String?) {
                    try {
                        snapshot.getValue(ChatListResponseItemRemote::class.java)
                            ?.let { updatedItem ->
                                val index =
                                    chatList.indexOfFirst { it.chatId == updatedItem.chatId }
                                if (index != -1) {
                        ...
                        Log.d(
                            "pshiiisagi",
                            "excepionwq:  ${e.message} and ${e.localizedMessage} and ${e.stackTrace} and ${e.printStackTrace()}"
                        )
                    }
                }
                ...
                override fun onCancelled(databaseError: DatabaseError) {
                    ...
                    Log.d(
                        "pshiiisagi",
                        "onCancelled: ${databaseError.toException()} and ${databaseError.message}"
                    )
    ...
    override suspend fun getUserList(): Flow<ResultState<UserListItem>> {
        try {
            val userList = mutableListOf<UserItemRemote>()
            val querySnapshot = firestore.collection("users").get().await()
            for (document in querySnapshot.documents) {
                val userItem = document.toObject(UserItemRemote::class.java)
                userItem?.let {
                    userList.add(it)
                }
            }
            val userListItem = UserListItemRemote(userList, userList.size)
            ResultState.Success(userListItem.copy(userList, userList.size))
        } catch (e: Exception) {
//            ResultState.Error(apiExceptionHandler.traceErrorException(e))
        }
